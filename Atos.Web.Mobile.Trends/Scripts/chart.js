            ...
            var canvas = this,
                ...
                drawSlice = function (canvas, d, context, slice) {
                    ...
                    if (slice == currentPullOutSlice) {
                        ...
                        context.fillText(defaults.pullOutValuePrefix + s['value'] + defaults.pullOutValueSuffix + " (" + (parseInt(s['value'] / totalValue * 100 + .5)) + "%)", centreX + Math.cos(midAngle) * (chartRadius + defaults.maxPullOutDistance + defaults.pullOutLabelPadding), centreY + Math.sin(midAngle) * (chartRadius + defaults.maxPullOutDistance + defaults.pullOutLabelPadding) + 20);
